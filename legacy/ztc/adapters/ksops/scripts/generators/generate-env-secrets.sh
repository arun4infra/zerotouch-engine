...
# Generate environment-specific secrets (DEV_*, STAGING_*, PROD_*, PR_*)
# Creates secrets with environment-specific prefixes
...
echo -e "${BLUE}Processing ${ENV_PREFIX} environment...${NC}"
...
# Function to get secret mapping
get_secret_mapping() {
    local var_name="$1"
    case "$var_name" in
...
kind: ksops
metadata:
  name: ${ENV_PREFIX}-secrets-generator
  annotations:
...
EOF
    ...
    echo -e "${YELLOW}  ⚠️  No ${ENV_PREFIX}_* variables found${NC}"
fi
echo ""
