...
class CertManagerScripts(str, Enum):
    ...
    # Post-work (1 script)
    WAIT_CERT_MANAGER = "post_work/wait-cert-manager.sh"
    
    # Validation (1 script)
    VALIDATE_CERT_MANAGER = "validation/validate-cert-manager.sh"
...
class CertManagerAdapter(PlatformAdapter):
    ...
    def get_required_inputs(self) -> List[InputPrompt]:
        ...
        return [
            ...
            InputPrompt(
                ...
                help_text="Kubernetes namespace for cert-manager"
            ),
            InputPrompt(
    ...
    def post_work_scripts(self) -> List[ScriptReference]:
        ...
        return [
            ScriptReference(
                package="workflow_engine.adapters.cert-manager.scripts",
                resource=CertManagerScripts.WAIT_CERT_MANAGER,
                description="Wait for cert-manager pods",
                timeout=300,
                context_data={
    ...
    def validation_scripts(self) -> List[ScriptReference]:
        ...
        return [
            ScriptReference(
                ...
                description="Validate cert-manager health",
                timeout=60,
                context_data={
                    "namespace": config.namespace
                }
    ...
    async def render(self, ctx: 'ContextSnapshot') -> AdapterOutput:
        """Generate cert-manager ArgoCD Application manifest"""
        config = CertManagerConfig(**self.config)
        ...
        app_template = self.jinja_env.get_template("cert-manager/application.yaml.j2")
        manifests["argocd/overlays/main/core/01-cert-manager.yaml"] = await app_template.render_async(**template_ctx)
