...
class AgentSandboxAdapter(PlatformAdapter):
    ...
    def get_required_inputs(self) -> List[InputPrompt]:
        ...
        return [
            InputPrompt(
                ...
                help_text="Container registry for agent-sandbox controller image"
            ),
            InputPrompt(
                name="image_tag",
                prompt="Image Tag",
                type="string",
                default="latest-main",
                help_text="Image tag for agent-sandbox controller"
            ),
            InputPrompt(
                name="namespace",
                prompt="Namespace",
                ...
                help_text="Kubernetes namespace for agent-sandbox controller"
            ),
            InputPrompt(
                name="mode",
    ...
    def pre_work_scripts(self) -> List[ScriptReference]:
        """Pre-work scripts (none for agent-sandbox)"""
        return []
    ...
    def post_work_scripts(self) -> List[ScriptReference]:
        """Post-work scripts (none for agent-sandbox)"""
        return []
    ...
    async def render(self, ctx: 'ContextSnapshot') -> AdapterOutput:
        ...
        app_template = self.jinja_env.get_template("agent-sandbox/application.yaml.j2")
        manifests["argocd/base/04-agent-sandbox.yaml"] = await app_template.render_async(**template_ctx)
        
        # Render wrapper kustomization
        wrapper_template = self.jinja_env.get_template("agent-sandbox/wrapper/kustomization.yaml.j2")
