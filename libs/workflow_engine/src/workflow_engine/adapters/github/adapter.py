...
class GithubAdapter(PlatformAdapter):
    ...
    def get_required_inputs(self) -> List[InputPrompt]:
        ...
        return [
            ...
            InputPrompt(
                ...
                help_text="Format: https://github.com/org/repo.git (must end with .git)"
            ),
            InputPrompt(
    ...
    async def render(self, ctx: 'ContextSnapshot') -> AdapterOutput:
        ...
        # Extract org and repo from control plane URL (strip .git suffix)
        import re
        match_cp = re.match(r"^https://github\.com/([^/]+)/([^/]+?)(\.git)?/?$", config.control_plane_repo_url.rstrip('/'))
        cp_org = match_cp.group(1)
        cp_repo = match_cp.group(2)  # Without .git suffix
        
        # Extract org and repo from data plane URL (strip .git suffix)
        match_dp = re.match(r"^https://github\.com/([^/]+)/([^/]+?)(\.git)?/?$", config.data_plane_repo_url.rstrip('/'))
        dp_org = match_dp.group(1)
        dp_repo = match_dp.group(2)  # Without .git suffix
        
        # Generate GHCR token refresher CronJob
        manifests = {}
        ghcr_template = self.jinja_env.get_template("github/ghcr-token-refresher.yaml.j2")
        manifests["argocd/k8/foundation/ghcr-token-refresher/cronjob.yaml"] = await ghcr_template.render_async()
