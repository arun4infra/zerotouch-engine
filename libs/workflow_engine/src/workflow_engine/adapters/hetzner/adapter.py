...
class HetznerAdapter(PlatformAdapter):
    ...
    def get_required_inputs(self) -> List[InputPrompt]:
        ...
        return [
            ...
            InputPrompt(
                ...
                type="password",
                help_text="64-character hex string for DNS management (cert_manager, external_dns)"
            ),
            InputPrompt(
                name="server_ips",
    ...
    async def render(self, ctx: 'ContextSnapshot') -> AdapterOutput:
        ...
        # COMMENTED OUT: Secrets now managed by KSOPS adapter in core overlay
        # manifests = {}
        ...
        #     manifests[f"{secrets_path}/external_dns-hetzner.secret.yaml"] = self._render_secret(
        #         name="external_dns-hetzner",
        #         namespace="kube-system",
        ...
        #         "./hcloud.secret.yaml",
        #         "./external_dns-hetzner.secret.yaml"
        #     ])
        #     
        #     # Kustomization
        ...
        manifests = {}  # Empty manifests - secrets handled by KSOPS adapter
        
        # Generate ArgoCD Applications for Hetzner components
        hcloud_ccm_template = self.jinja_env.get_template("hetzner/hcloud-ccm.yaml.j2")
        manifests["argocd/k8/core/01-hcloud-ccm.yaml"] = await hcloud_ccm_template.render_async()
        
        cert_webhook_template = self.jinja_env.get_template("hetzner/cert-manager-webhook-hetzner.yaml.j2")
        manifests["argocd/k8/core/01-cert-manager-webhook-hetzner.yaml"] = await cert_webhook_template.render_async()
