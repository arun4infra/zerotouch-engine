...
if [[ -d "$CACHE_DIR/.git" ]]; then
    ...
    git remote set-url origin "$TENANT_REPO_AUTH_URL"
...
# Verify environment file exists (or will be created)
if [[ ! -f "$ENV_FILE" ]]; then
    echo "Warning: Environment config not found: $ENV (will be created)" >&2
    echo "Path: $ENV_FILE" >&2
    # Don't exit - let the caller create it
fi

echo "✓ Tenant repo ready: $ENV" >&2
