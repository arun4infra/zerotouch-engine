#!/usr/bin/env bash
# Convert ~/.ztp/secrets to .env format
# Reads INI format from ~/.ztp/secrets and outputs .env format

set -euo pipefail

SECRETS_FILE="${HOME}/.ztp/secrets"

if [ ! -f "$SECRETS_FILE" ]; then
    echo "Error: ~/.ztp/secrets not found" >&2
    exit 1
fi
