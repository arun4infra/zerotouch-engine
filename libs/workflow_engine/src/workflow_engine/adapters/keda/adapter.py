...
class KEDAAdapter(PlatformAdapter):
    ...
    def post_work_scripts(self) -> List[ScriptReference]:
        ...
        return [
            ScriptReference(
                package="workflow_engine.adapters.keda.scripts",
    ...
    async def render(self, ctx: 'ContextSnapshot') -> AdapterOutput:
        ...
        manifests["argocd/base/04-keda.yaml"] = await app_template.render_async(**template_ctx)
    ...
    def get_stage_context(self, stage_name: str, all_adapters_config: Dict[str, Any]) -> Dict[str, Any]:
        """Return non-sensitive context for KEDA bootstrap stages"""
