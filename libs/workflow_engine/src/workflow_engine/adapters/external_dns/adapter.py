...
class ExternalDNSScripts(str, Enum):
    ...
    # Post-work (1 script)
    WAIT_EXTERNAL_DNS = "post_work/wait-external_dns.sh"
    
    # Validation (1 script)
    VALIDATE_EXTERNAL_DNS = "validation/validate-external_dns.sh"
...
class ExternalDnsAdapter(PlatformAdapter):
    ...
    def post_work_scripts(self) -> List[ScriptReference]:
        ...
        return [
            ScriptReference(
                package="workflow_engine.adapters.external_dns.scripts",
    ...
    async def render(self, ctx: 'ContextSnapshot') -> AdapterOutput:
        ...
        app_template = self.jinja_env.get_template("external_dns/application.yaml.j2")
        manifests["argocd/k8/core/01-external-dns.yaml"] = await app_template.render_async(**template_ctx)
